...
func SeedComponents(ctx context.Context, s ComponentStore, st storage.Client) error {
	manifest, err := componentlib.LoadManifest()
	if err != nil {
		return fmt.Errorf("bootstrap: load component manifest: %w", err)
	}

	for _, cat := range componentlib.ExtractCategories(manifest) {
...
func uploadComponentIcons(ctx context.Context, s ComponentStore, st storage.Client, manifest []componentlib.Component) error {
	iconBySlug := map[string]string{}
	for _, c := range manifest {
		iconBySlug[componentlib.IconSlug(c)] = c.ID
	}
