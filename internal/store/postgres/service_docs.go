...
func (d *DB) CreateServiceDoc(ctx context.Context, sd catalog.ServiceDoc) error {
	const q = `
		INSERT INTO service_docs
			(service_id, doc_id, org_id, created_by, updated_by, created_at, updated_at)
...
func (d *DB) ListServiceDocs(ctx context.Context, serviceID string) ([]catalog.ServiceDoc, error) {
	const q = `
		SELECT
			sd.service_id, sd.doc_id, sd.org_id, sd.created_by, sd.updated_by, sd.created_at, sd.updated_at, sd.deleted_at,
	...
	for rows.Next() {
		...
		var doc docs.Doc
		if err := rows.Scan(
			&sd.ServiceID, &sd.DocID, &sd.OrgID, &sd.CreatedBy, &sd.UpdatedBy, &sd.CreatedAt, &sd.UpdatedAt, &sd.DeletedAt,
