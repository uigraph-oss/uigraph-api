...
func (d *DB) CreateServiceDiagram(ctx context.Context, sd catalog.ServiceDiagram) error {
	const q = `
		INSERT INTO service_diagrams
			(service_id, diagram_id, org_id, created_by, updated_by, created_at, updated_at)
...
func (d *DB) GetServiceDiagram(ctx context.Context, serviceID, diagramID string) (*catalog.ServiceDiagram, error) {
	const q = `
		SELECT service_id, diagram_id, org_id, created_by, updated_by, created_at, updated_at, deleted_at
...
func (d *DB) ListServiceDiagrams(ctx context.Context, serviceID string) ([]catalog.ServiceDiagram, error) {
	const q = `
		SELECT
			sd.service_id, sd.diagram_id, sd.org_id, sd.created_by, sd.updated_by, sd.created_at, sd.updated_at, sd.deleted_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&sd.ServiceID, &sd.DiagramID, &sd.OrgID, &sd.CreatedBy, &sd.UpdatedBy, &sd.CreatedAt, &sd.UpdatedAt, &sd.DeletedAt,
