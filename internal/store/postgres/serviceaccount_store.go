...
const saCols = `id, org_id, name, COALESCE(description,''), scopes, disabled, is_internal,
	avatar_asset_id,
...
func scanSA(row interface{ Scan(...any) error }) (identity.ServiceAccount, error) {
	...
	err := row.Scan(
		&sa.ID, &sa.OrgID, &sa.Name, &sa.Description, pq.Array(&sa.Scopes), &sa.Disabled, &sa.IsInternal,
		&avatarAssetID,
...
func (d *DB) CreateServiceAccount(ctx context.Context, sa identity.ServiceAccount) error {
	const q = `
		INSERT INTO service_accounts
		    (id, org_id, name, description, scopes, disabled, is_internal, created_by, created_at, updated_at)
...
func (d *DB) GetSystemServiceAccount(ctx context.Context, orgID string) (*identity.ServiceAccount, error) {
	q := "SELECT " + saCols + " FROM service_accounts WHERE org_id=$1 AND name=$2 AND is_internal=TRUE AND deleted_at IS NULL"
