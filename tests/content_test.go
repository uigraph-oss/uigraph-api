...
func TestDiagrams_Sync_Create(t *testing.T) {
	name := fmt.Sprintf("sync-create-%d", time.Now().UnixNano())

	body := mustDo(t, "POST", "/api/v1/orgs/"+orgID+"/diagrams/sync", adminToken, M{
		"name": name, "content": sampleContent,
	})
	id := str(body, "diagramId")
	if id == "" {
		t.Fatal("want diagramId in sync response")
	}
	if body["versionCreated"] != true {
		t.Fatalf("want versionCreated=true on create, got %v", body["versionCreated"])
	}

	// Verify diagram was created with correct content.
	cBody := mustDo(t, "GET", "/api/v1/orgs/"+orgID+"/diagrams/"+id+"/content", adminToken, nil)
	if cBody["content"] != sampleContent {
		t.Fatalf("want sampleContent after sync create, got %v", cBody["content"])
	}

	do("DELETE", "/api/v1/orgs/"+orgID+"/diagrams/"+id, adminToken, nil)
}

func TestDiagrams_Sync_Update_ContentChanged(t *testing.T) {
	// Seed a diagram.
	initial := mustDo(t, "POST", "/api/v1/orgs/"+orgID+"/diagrams/sync", adminToken, M{
		"name": fmt.Sprintf("sync-upd-%d", time.Now().UnixNano()), "content": sampleContent,
	})
	id := str(initial, "diagramId")
	...
	if body["versionCreated"] != true {
		t.Fatalf("want versionCreated=true when content changed, got %v", body["versionCreated"])
	}

	// Content updated.
	cBody := mustDo(t, "GET", "/api/v1/orgs/"+orgID+"/diagrams/"+id+"/content", adminToken, nil)
	if cBody["content"] != updatedContent {
		t.Fatalf("want updatedContent after sync update, got %v", cBody["content"])
	}

	// Two versions now (v1 from create, v2 from sync).
	vBody := mustDo(t, "GET", "/api/v1/orgs/"+orgID+"/diagrams/"+id+"/versions", adminToken, nil)
	if n := len(list(vBody, "versions")); n != 2 {
		t.Fatalf("want 2 versions after sync update, got %d", n)
	}

	do("DELETE", "/api/v1/orgs/"+orgID+"/diagrams/"+id, adminToken, nil)
}

func TestDiagrams_Sync_Update_ContentUnchanged(t *testing.T) {
	// Seed a diagram.
	initial := mustDo(t, "POST", "/api/v1/orgs/"+orgID+"/diagrams/sync", adminToken, M{
		"name": fmt.Sprintf("sync-same-%d", time.Now().UnixNano()), "content": sampleContent,
	})
	id := str(initial, "diagramId")
	...
	if body["versionCreated"] != false {
		t.Fatalf("want versionCreated=false when content unchanged, got %v", body["versionCreated"])
	}

	// Still only 1 version (the one from the initial create).
	vBody := mustDo(t, "GET", "/api/v1/orgs/"+orgID+"/diagrams/"+id+"/versions", adminToken, nil)
	if n := len(list(vBody, "versions")); n != 1 {
		t.Fatalf("want 1 version when content unchanged, got %d", n)
	}
